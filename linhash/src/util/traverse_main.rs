...
impl TraverseMain<'_> {
    ...
    pub fn exec(self) -> Result<u64> {
        let n_simple = self.traverse_main_pages_simple()?;
        if n_simple < 2 {
            return Ok(n_simple);
        }
        ...
        let last_page = self.db.main_pages.read_page_ref(last_page_id)?.unwrap();
        let locallevel = last_page.locallevel().unwrap();
        let old_page_id = calc_old_page_id(last_page_id, locallevel);
        let old_page = self.db.main_pages.read_page_ref(old_page_id)?.unwrap();
    ...
    pub fn traverse_main_pages_simple(&self) -> Result<u64> {
        let page0 = self.db.main_pages.read_page_ref(0)?;
        if page0.is_none() {
            return Ok(0);
        }

        let page1 = self.db.main_pages.read_page_ref(1)?;
        if page1.is_none() {
            return Ok(1);
        }

        for page_id in 2.. {
            let Some(_) = self.db.main_pages.read_page_ref(page_id)? else {
                return Ok(page_id);
            };
        }
